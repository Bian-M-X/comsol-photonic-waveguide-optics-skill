...
def load_component_data(manifest_path: Path, name: str, component: dict[str, Any]) -> ComponentData:
    np = import_numpy()
    csv_path = (manifest_path.parent / component["sparameters"]).resolve()
    try:
    ...
    for line_number, row in enumerate(raw_rows, start=2):
        try:
            ...
            in_port = row["in_port"]
            value = complex(float(row["s_real"]), float(row["s_imag"]))
...
def load_all_component_data(manifest_path: Path, manifest: dict[str, Any]) -> dict[str, ComponentData]:
    data = {
        name: load_component_data(manifest_path, name, component)
...
def validate_manifest(manifest_path: Path, check_data: bool = True) -> tuple[dict[str, Any], dict[str, ComponentData]]:
    manifest_path = manifest_path.resolve()
    manifest = load_manifest(manifest_path)
    errors = validate_structure(manifest)
    if errors:
        raise AssemblyError("manifest validation failed:\n- " + "\n- ".join(errors))
    component_data = load_all_component_data(manifest_path, manifest) if check_data else {}
    return manifest, component_data


def compose(manifest: dict[str, Any], component_data: dict[str, ComponentData]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    np = import_numpy()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        manifest, component_data = validate_manifest(args.manifest, check_data=not getattr(args, "structure_only", False))
        if args.command == "validate":
            print(json.dumps({"valid": True, "manifest": str(args.manifest.resolve()), "data_checked": not args.structure_only}, indent=2))
            return 0
